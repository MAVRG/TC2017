...
class RosTensorFlow():
	...
	self.sub_twist_flag=0
        ...
        self._sub = rospy.Subscriber('/image_color', Image, self.callback0, queue_size=1)
        self._sub1 = rospy.Subscriber('/twist_cmd', TwistStamped, self.callback1)
        self._pub = rospy.Publisher('/joy/CNN2/cmd_vel', Twist, queue_size=1)
        ...
        self._saver.restore(self._session, "model/20170905model.ckpt")

    def callback(self):

       if(self.sub_image0_flag==1 and self.sub_twist_flag==1):

            self.sub_image0_flag=0
            self.sub_twist_flag=0
            list_image = []
            cv_image = self._cv_bridge.imgmsg_to_cv2(self.image_msg0, "bgr8")
   	    cv_image = cv_image[200:1240,200:1720,:]
	    ...
	    label = [[x/l,y/l]]
            ...
            self._stre = 0.2*self._session.run(self.y_conv, feed_dict={self.x:list_image,self.label:label})
            self._ac = 0.2
            twist = Twist()
            stre = self._stre
            ac = self._ac
