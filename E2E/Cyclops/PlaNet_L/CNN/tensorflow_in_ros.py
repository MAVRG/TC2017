...
class RosTensorFlow():
	...
	self.sub_twist_flag=0
        ...
        self._sub = rospy.Subscriber('/image_color', Image, self.callback_image)
        self._sub1 = rospy.Subscriber('/cyclops_twist_cmd', TwistStamped, self.callback_twist)
        ...
        self._saver.restore(self._session, "model/20171105model.ckpt")#"model/20170905model.ckpt")
        print("read")
    ...
    def callback(self):
       ...
       if(self.sub_image0_flag==1 and self.sub_twist_flag==1):
            ...
            cv_image = cv_image[473:967,210:1710,:]
            ...
            self._stre = self._session.run(self.y_conv, feed_dict={self.x:list_image,self.label:label})
            self._ac = 0.2
            twist = Twist()
            stre = self._stre
            ac = self._ac
