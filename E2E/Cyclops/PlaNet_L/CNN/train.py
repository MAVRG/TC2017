...
def make_dataset(filenames):
	...
	for filename in filenames:
		degree = ["-20","-15","-10","-5","0","5","10","15","20"]
		for deg in degree:
			...
			random_seq = range(len(image_file))
			random.shuffle(random_seq)

			for l in range(len(image_file)/5):
			  image = cv2.imread(image_file[random_seq[l]])
			  train_image.append(image)
			  train_r.append(r[random_seq[l]][1])
			  train_label.append(label[random_seq[l]])
...
while(l<=200000):
 filenames = ["/media/kerberos/Samsung_T31/20170803traintrain"]
 train_image,train_r,train_label = make_dataset(filenames)

 filenames = ["/media/kerberos/Samsung_T31/20170803testtrain"]
 ...
 for i in range(itter):
  l += 1
  sess.run(train_step,feed_dict={image1:train_image[random_seq[i*batchSize:(i+1)*batchSize],:,:,:],label1:train_label[random_seq[i*batchSize:(i+1)*batchSize],:], y_r:train_r[random_seq[i*batchSize:(i+1)*batchSize],:], keep_prob: 0.5})
  ...
  if(l%100 == 0):
       rmse = sess.run(loss, feed_dict={image1: test_image,label1:test_label, y_r: test_r,keep_prob:1.0})
       f = open("loss/model20170818.txt","a")
       #f = open("loss/model20170613.txt","a")#kanbayashi
       if(rmse<trainrmse):    
           trainrmse=rmse
           save_path = saver.save(sess, "model/20170818model.ckpt")
