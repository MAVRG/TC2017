...
int SSM_get_pos_GL_time(double time, double *x, double *y, double *theta)
{
  ...
  tid = readSSM(odm_gl_sid, (char*)&odm2, &time2, tid+1);
  if(tid < 0){/*次のがなければ前のでなんとか。*/
    time2 = time1;
    odm2 = odm1;
    tid = readSSM(odm_gl_sid, (char*)&odm1, &time1, tid-1);
  }
  if(fabs(time2 - time1) < 0.000001)return 0;
  if(fabs(time2 - time1) > 0.03)return 0;
  ...
  *y = odm1.y+ (odm2.y - odm1.y)*rate;
  while(odm2.theta - odm1.theta <-M_PI)odm2.theta+=2.0*M_PI;
  while(odm2.theta - odm1.theta > M_PI)odm2.theta-=2.0*M_PI;
...
int SSM_get_pos_BS_time(double time, double *x, double *y, double *theta)
{
  ...
  tid = readSSM(odm_bs_sid, (char*)&odm2, &time2, tid+1);
  if(tid < 0){/*次のがなければ前のでなんとか。*/
    time2 = time1;
    odm2 = odm1;
